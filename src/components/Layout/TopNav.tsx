...
export function TopNav({ selectedOrgId, onOrgChange, searchQuery, onSearchChange }: TopNavProps) {
  ...
  return (
    <nav style={{ backgroundColor: '#0b1a30', borderBottom: '1px solid #27425f' }}
      className="px-6 py-3 flex items-center justify-between gap-3">

      {/* Brand */}
      <div className="flex-shrink-0">
        <div className="text-white font-bold text-xl leading-none tracking-tight">UPS</div>
        <div style={{ color: '#6c7f9c' }} className="text-xs mt-0.5">APC uninterruptible power supplies</div>
      </div>

      {/* Search */}
      <div className="relative flex-1 max-w-xs">
        <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#6c7f9c' }} />
        <input
          type="text"
          placeholder="Search unit, MAC, IP, room..."
          value={searchQuery}
          onChange={e => onSearchChange(e.target.value)}
          style={{ backgroundColor: '#0c1b30', border: '1px solid #27425f', color: '#eef3fb' }}
          className="w-full rounded-lg pl-8 pr-3 py-1.5 text-sm outline-none placeholder-slate-600"
        />
      </div>

      {/* Live indicator */}
      <div style={{ backgroundColor: '#0d1f1a', border: '1px solid #1a3a2a', color: '#34d17e' }}
      ...
      <button
        ...
        style={{ backgroundColor: '#f9a52e', color: '#000' }}
      ...
      <div className="relative flex-shrink-0">
        <button
          ...
          <Home size={13} style={{ color: '#6c7f9c' }} />
        ...
        {dropdownOpen && (
          <div style={{ backgroundColor: '#0c1b30', border: '1px solid #27425f' }}
            ...
            {/* All orgs option */}
            <button
              style={{ color: !selectedOrgId ? '#4d8df5' : '#eef3fb', borderBottom: '1px solid #27425f' }}
            ...
            {orgs.length === 0 ? (
              <div style={{ color: '#6c7f9c' }} className="px-3 py-3 text-sm text-center">
              ...
              orgs.map(org => (
                <button key={org.id}
                  style={{ color: selectedOrgId === org.id ? '#4d8df5' : '#eef3fb' }}
                  ...
                  <div style={{ color: '#6c7f9c' }} className="text-xs truncate">{org.label}</div>
                </button>
              ))
            )}
          </div>
      ...
      {/* Bell */}
      <button className="relative flex-shrink-0" style={{ color: '#6c7f9c' }}>
        <Bell size={20} />
        {activeCount > 0 && (
      ...
      {/* Dark toggle */}
      <button style={{ color: '#6c7f9c', backgroundColor: '#0c1b30', border: '1px solid #27425f' }}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm flex-shrink-0">
        <Moon size={13} />
        <span style={{ color: '#eef3fb' }}>Light</span>
      </button>
